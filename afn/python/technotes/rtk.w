...
When a client first connects, it sends what's called the request line. Right now, this contains two keys, action and features, the former of which contains the string "request" and the latter of which is a list of strings representing the features the client supports, but in the future this may contain additional keys for specifying a particular application to show (thereby allowing multiple applications to be served from a single port) or other such request parameters.

The server then responds with the response line, which contains two keys, which are the same format as the request, except that the action will be "response". If the server decides that the client doesn't support enough features to run the application on even a basic level, it can instead send an error action as described above, after which it must drop the connection (and the client will do so even if the server does not). An error that occurs after the response is sent is not deemed a fatal error, whereas one sent instead of the response is.
...
= Containers =
== HBox ==
This container lays its contents out horizontally. Components are laid out left to right. [[some/page.w|Hello world]]
